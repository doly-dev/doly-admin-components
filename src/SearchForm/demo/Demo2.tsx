 ...
 * title: 审核记录查询
 * desc: |
 *  依赖了通用组件 `Dictionary`，请自行拷贝到项目中引入。
 * 
 *  审核状态中仅包含 `审核通过` `审核拒绝`
 */
...
const initialValues = {
  code: "", // 申请编号
  applicantName: "", // 申请人姓名（经办员）
  approverName: "", // 审核人姓名
  approveResult: "2,3", // 审核结果
...
export default ({ onSubmit = () => { }, name = "approve", submitOnMount = false, loading = false, defaultValues = {} }: CompProps) => {
    ...
    <Card style={{ paddingTop: 24 }}>
      ...
      >
        <Row>
          ...
          <Col {...colLayout}>
            <Form.Item label="经办员" name="applicantName">
              <Input allowClear placeholder="请输入" />
            </Form.Item>
          </Col>
          <Col {...colLayout}>
          ...
          <Col {...colLayout}>
            <Form.Item label="审核状态" name="approveResult">
              <Dictionary.Select data={enumApproveResult} allValue="2,3" excludeValues={[1]} />
            </Form.Item>
          </Col>
        </Row>
        <Row>
